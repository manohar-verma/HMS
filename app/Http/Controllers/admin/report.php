...
class report extends BaseController {
    ...
    function index()
    {
        ...
        if(!in_array("occupancy", $accessRole))
        {
    ...
    function revenue(){
        ...
        $userAccess = $this->_myFunUserRole->getUserAccess($user->id);
        $accessRole=(explode(",",$userAccess));
        if(!in_array("revenue", $accessRole))
        {
             Session::put('error', 'Access denied to this page');
            return redirect(ADMIN_URL.'/dashboard');
        } 
    ...
    function cancellation(){
        ...
        $userAccess = $this->_myFunUserRole->getUserAccess($user->id);
        $accessRole=(explode(",",$userAccess));
        if(!in_array("cancellation", $accessRole))
        {
             Session::put('error', 'Access denied to this page');
            return redirect(ADMIN_URL.'/dashboard');
        } 
        return view('admin.report.cancellation');
    }
    function bookingSources(){
        $user = auth()->guard('admin')->user();
        $userAccess = $this->_myFunUserRole->getUserAccess($user->id);
        $accessRole=(explode(",",$userAccess));
        if(!in_array("booking-sources", $accessRole))
        {
             Session::put('error', 'Access denied to this page');
            return redirect(ADMIN_URL.'/dashboard');
        } 
        return view('admin.report.sources');
    }
