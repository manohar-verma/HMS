(function($){
    $.fn.extend({
        ...
        passwordCheck: function(options){
            var alphaNumericReg = new RegExp(/(?=.*\d)(?=.*[a-z])(?=.*[@$!%*#?&^_-]).{8,}/);
            var defaults = {
                errorArea: '',
                alphaNumericCk: false,
                minLen:0,
                maxLen:0,
            ...
            if ($(this).length == 1) {
                ...
                else if ((options_v.minLen > 0) & (nLen<options_v.minLen)) {
                    this.ErrerAdd($(this),options_v.errorArea,'Password should must be at least '+options_v.minLen+' characters');
                    return false;
                }
                else if ((options_v.maxLen > 0) & (nLen>options_v.maxLen)) {
                    this.ErrerAdd($(this),options_v.errorArea,'Password must be '+options_v.minLen+'-'+options_v.maxLen+' characters long');
                    return false;
                }
                else if ((options_v.alphaNumericCk) & (!alphaNumericReg.test(ThisValue))) {
                    this.ErrerAdd($(this),options_v.errorArea,'Password must contain at least one letter, one number and one special character');
                    return false;
                }
                else{
                   this.ErrerFree($(this),options_v.errorArea);
