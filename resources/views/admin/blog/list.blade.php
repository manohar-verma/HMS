...
<div class="container-fluid">
    <div class="row page-titles">
        <div class="col-md-5 col-12 align-self-center">
            <h3 class="text-themecolor mb-0">Blog</h3>
        </div>
        <div class="col-md-7 col-12 align-self-center d-none d-md-flex justify-content-end">
            <ol class="breadcrumb mb-0 p-0 bg-transparent">
                <li class="breadcrumb-item"><a href="javascript:void(0)">Home</a></li>
                <li class="breadcrumb-item active d-flex align-items-center">Blog</li>
            </ol>
        </div>
    </div>
    <!-- -------------------------------------------------------------- -->
        ...
        <div class="card card-body">
            <div class="row">
                                ...
                                <div class="input-group">
                                    ...
                                    <div class="input-group-append">
                                        <button type="submit" class="btn btn-success rounded-pill px-4"><span
                                            ...
                                            <button type="button" onclick="window.location='{{ADMIN_URL}}/blog'"
                ...
                <div
                    ...
                    <a href="{{ADMIN_URL}}/blog/create" id="btn-add-contact" class="btn btn-info">
        ...
        <div class="card card-body">
            <div class="table-responsive">
                <table class="table search-table v-middle">
                    ...
                    <th style="cursor:pointer;" onClick="handleFilter('title','{{$nextSortDirection}}','{{$currentSortFiled}}');">Title @if($currentSortFiled=='title') <i class="bi-caret-{{$currentSortDirection}}-square-fill" ></i> @endif</th>
                    <th style="cursor:pointer;" onClick="handleFilter('author','{{$nextSortDirection}}','{{$currentSortFiled}}');">Author @if($currentSortFiled=='author') <i class="bi-caret-{{$currentSortDirection}}-square-fill" ></i> @endif</th>
                    <th style="cursor:pointer;" onClick="handleFilter('date','{{$nextSortDirection}}','{{$currentSortFiled}}');">Date @if($currentSortFiled=='date') <i class="bi-caret-{{$currentSortDirection}}-square-fill" ></i> @endif</th>
                        <th>Category</th>
                        <th>Tags</th>
                        <th>Action</th>
                    </thead>
                    <tbody>
                        ...
                        <?php 
                        $catData = $get_site_details->getBlogCat($listitem->catagory);
                        $authorData = $get_site_details->getAuthor($listitem->author);
                        ?>
                        <!-- row -->
                        <tr class="search-items">
                            ...
                            <td>
                               @if(!empty($authorData))  {{$authorData->first_name}} {{$authorData->last_name}} @endif
                            </td>
                            <td>
                                 {{date('d-m-Y',strtotime($listitem->date))}}
                            </td>
                            <td>
                                @if(!empty($catData)) {{$catData->name}} @endif
                            </td>
                            <td>
                                 {{$listitem->tags}}
                            </td>
                            ...
                            <td>
                                ...
                                <div class="action-btn">
                                    <a href="{{ADMIN_URL}}/blog/{{$listitem->id}}/edit" class="text-info edit"><i data-feather="edit"
                                            class="feather-sm fill-white"></i></a>
                                    <a href="{{ADMIN_URL}}/blog/delete/{{$listitem->id}}" class="text-dark  ms-2" onclick="return confirm('Are you sure to delete?');"><i data-feather="trash-2"
                                            class="feather-sm fill-white"></i></a>
...
<script type="text/javascript">
  ...
  $(document).on('click','.changeStatus',function(){
	  ...
	  $.ajax({
		  type: "POST",
		  url: "{{ADMIN_URL}}/blog/changeStatus/"+id,
			data: { _token : _token },
