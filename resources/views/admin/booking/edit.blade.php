        ...
        <div class="col-sm-12">
            <div class="card">
                <div class="card-body">
                    ...
                    <h5 class="card-subtitle mb-3 border-bottom pb-3">Edit Booking</h5>
                <form id="addEditForm" class="form" action="{{ADMIN_URL}}/booking/new" method="POST" enctype="multipart/form-data">
                                ...
                                <div class="mb-3 row">
                                    ...
                                    <div class="col-md-10">
                                        <select class="form-select col-12" id="hotels" name="hotels" onChange="hotelSelect();">
                                            ...
                                            <option value="{{$item->hotel_id}}" @if($item->hotel_id = $bookingData->hotel_id) selected @endif>{{$item->name}} <i class="fas fa-chevron-circle-right"></i> {{$item->address}},{{$item->phone}}</option>
                                            @endforeach
                                    ...
                                    <label for="guest" class="col-md-2 col-form-label">Guest {!!REQUIRED_STAR!!}</label>
                                        ...
                                        <div class="col-md-5">
                                            ...
                                            <option value="">Choose Guest</option>
                                                ...
                                                <option value="{{$item->id}}"  @if($item->id = $bookingData->guest_id) selected @endif>{{$item->name}} - {{$item->phone}}</option>
                                                @endforeach     
                                            </select>
                                        </div>
                                ...
                                <div class="mb-3 row">
                                        ...
                                        <div class="col-md-10">
                                            <input class="datepicker form-control"  placeholder="dd-mm-yyyy" type="text" type="text" value="@if(!empty($bookingData->check_in_date)){{date('d-m-Y',strtotime($bookingData->check_in_date))}}@endif"
                                ...
                                <div class="mb-3 row">
                                        ...
                                        <div class="col-md-10">
                                            <input class="datepicker form-control"  placeholder="dd-mm-yyyy" type="text" type="text" value="@if(!empty($bookingData->check_out_date)){{date('d-m-Y',strtotime($bookingData->check_out_date))}}@endif"
                                ...
                                <div class="mb-3 row">
                                    ...
                                    <div class="col-md-5" id="number_of_days">
                                        <button type="button" class="btn d-flex btn-light-success w-100 d-block text-success font-weight-medium">
                                        Number of days <span class="badge ms-auto bg-success" id="count_value">{{$bookingDays}}</span>
                                        </button>
                                        <input type="hidden" name="bookingDayCount" id="bookingDayCount" value="{{$bookingDays}}">
                                    </div>
                                </div>
                                ...
                                <div class="mb-3 row">
                                        ...
                                        <div class="col-md-10">
                                            <input class="form-control allow_numeric"type="text" name="num_guests" id="num_guests"
                                                value="{{$bookingData->num_guests}}" min="1" onkeyup="handleKeyUp(this.value);">
                                ...
                                <div class="mb-3 row">
                                    ...
                                    <div class="col-md-10">
                                        <select class="form-select col-12" id="payment_method" name="payment_method">
                                            ...
                                            @foreach($paymentMethodArray as $key=>$item)
                                            <option value="{{$key}}" @if(!empty($paymentInfo) && $paymentInfo->payment_method == $key) selected @endif>{{$item}}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                                <div class="mb-3 row">
                                    ...
                                    <div class="col-md-10">
                                        <input class="form-control" type="text" placeholder="Payment Reference"
                                            ...
                                            value="@if(!empty($paymentInfo)) {{$paymentInfo->payment_ref}} @endif">
                                    </div>
                                </div>
                                <div class="mb-3 row">
                                    ...
                                    <div class="col-md-10">
                                        ...
                                        <div class="table-responsive">
                                           <table class="table table-bordered text-center align-middle" id="priceCal">
                                            ...
                                            <tbody>
                                                @foreach($bookedRoomInfo as $listitem)
                                                <?php $getRoomInfo = $get_site_details->getBookedRoomInfo($listitem->room_id);
                                                $getRoomTypeInfo = [];
                                                if(!empty($getRoomInfo))
                                                 $getRoomTypeInfo = $get_site_details->getRoomType($getRoomInfo->room_type_id);
                                                ?>
                                                <tr>
                                                <td>@if(!empty($getRoomInfo)){{$getRoomInfo->room_number}}@endif</td>
                                                <td>@if(!empty($getRoomTypeInfo)){{$getRoomTypeInfo->title}}@endif</td>
                                                <td>@if(!empty($getRoomInfo)){{$getRoomInfo->description}}@endif</td>
                                                <td>₹{{$listitem->room_base_price}}</td>
                                                <td>{{$bookingDays}}</td>
                                                <td>₹{{ number_format($bookingDays * $listitem->room_base_price, 2) }}</td>
                                                </tr>
                                                @endforeach
                                                <tr class="table-secondary fw-bold">
                                                <td colspan="5" class="text-end">Total Paid Amount</td>
                                                <td>₹@if(!empty($paymentInfo)){{$paymentInfo->amount}}@else 0 @endif</td>
                                                </tr>
                                            </tbody>
                                            </table>
