        ...
        <div class="col-sm-12">
            <div class="card">
                <div class="card-body">
                    ...
                    <h5 class="card-subtitle mb-3 border-bottom pb-3">Update user</h5>
                    <form id="addEditForm" class="form" action="{{ADMIN_URL}}/users/sub-admin/{{$userData->id}}" method="POST"
                        ...
                        <div class="mb-3 row">
                            ...
                            <div class="col-md-10">
                                <input class="form-control" type="text" name="email" id="email" value="{{$userData->email}}" readonly>
                            </div>
                        </div>
                        <div class="mb-3 row">
                                                ...
                                                <div class="form-check">
                                                    <label class="form-check-label" id="check_label_{{$keyAccess}}">
                                                        <input class="form-check-input" value="{{$access}}"
                                                            ...
                                                            @if($access=='change-password' ) disabled
                                                            @endif @if(in_array($access,$userAccessList)) checked @endif onclick="handleReportSelect('check_{{$keyAccess}}','check_level_{{str_replace('-','_',$access)}}','{{count($accessLevelArray)}}');">{{ucwords(str_replace('-',' ',$access))}}
                                                    </label>
                                                </div>
                                            </td>
                                                        ...
                                                        <div class="form-check">
                                                            <label class="form-check-label" id="check_level_{{$key}}" style="font-size: 14px;font-weight: 400;color:#0f5e20;">
                                                                <input class="form-check-input" value="{{$accessLevel}}"
                                                                    ...
                                                                    onclick="handleAccessSelect('check_{{$keyAccess}}','check_level_{{str_replace('-','_',$access)}}','{{$key}}','{{count($accessLevelArray)}}');" @if(in_array($accessLevel,$accessTypeList[$access])) checked @endif>{{ucwords(str_replace('-',' ',$accessLevel))}}
                                                            </label>
                                                        </div>
                                                    </td>
                                                @endforeach
                        ...
                        <div class="mb-3 row">
                            ...
                            <div class="col-md-6">
                                <span>Sub admin password change</span>
                        ...
                        <div class="mb-3 row">
                            ...
                            <div class="col-md-10">
                                <div class="form-check">
                                    ...
                                    @if($userData->status==1) checked="true" @endif>
                                    <label class="form-check-label" for="Active">Active</label>
                                </div>
                                <div class="form-check">
                                    <input type="radio" id="Deactivate" name="status" class="form-check-input"
                                        value="0" @if($userData->status==0) checked="true" @endif>
...
$(function() {
    $('#addEditForm').submit(function() {
        let passValidate = $('#password').val()!=''?$('#password').passwordCheck({alphaNumericCk: true,minLen:8,maxLen:16,}):true;
       console.log(passValidate);
        if ($('#name').commonCheck() & $('#email').validateEmail() & passValidate) {
            return true;
        }
        $('html, body').animate({
